...
test.describe("Monitoring workspace", () => {
  ...
  test("allocates most row width to heartbeat and RTT displays", async ({ page }) => {
    ...
    const sparkline = deviceCell.locator(".mon-mini-sparkline");
    ...
    const widths = await page.evaluate(() => {
      ...
      return {
        ...
        sparkline: document.querySelector(".mon-mini-sparkline")?.getBoundingClientRect().width ?? 0,
      };
    });

    expect(widths).not.toBeNull();
    expect(widths!.device).toBeGreaterThan(widths!.uptime * 10);
    expect(widths!.device).toBeGreaterThan(widths!.services * 7);
    expect(widths!.heartbeat).toBeGreaterThanOrEqual(320);
    expect(widths!.sparkline).toBeGreaterThanOrEqual(320);
    expect(widths!.services).toBeLessThanOrEqual(150);
  });
});
